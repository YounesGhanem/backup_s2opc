...
@IF NOT DEFINED ENABLE_TESTING (set ENABLE_TESTING=on)
@IF NOT DEFINED ENABLE_SAMPLES (set ENABLE_SAMPLES=on)
...
@REM @IF NOT DEFINED VS_VERSION (set VS_VERSION="Visual Studio 15 2017 Win64")
 @IF NOT DEFINED VS_VERSION (set VS_VERSION="Visual Studio 17 2022")
@REM @IF NOT DEFINED CONFIG (set CONFIG="Release")
...
@IF not %ERRORLEVEL% == 0 goto :EXIT_FAIL
@
@REM cmake -DBUILD_SHARED_LIBS=%BUILD_SHARED_LIBS% -DCMAKE_WINDOWS_EXPORT_ALL_SYMBOLS=%BUILD_SHARED_LIBS% -DS2OPC_PUBSUB_ONLY=on -DS2OPC_CLIENTSERVER_ONLY=off -DWITH_PYS2OPC=%WITH_PYS2OPC% -DPYS2OPC_WHEEL_NAME=%PYS2OPC_WHEEL_NAME% -DS2OPC_NANO_PROFILE=%S2OPC_NANO_PROFILE% -DS2OPC_EVENT_MANAGEMENT=%S2OPC_EVENT_MANAGEMENT% -DS2OPC_NODE_MANAGEMENT=%S2OPC_NODE_MANAGEMENT% -DCMAKE_PREFIX_PATH=%CMAKE_LIBS_DIRS% -DMBEDTLS_INCLUDE_DIR=%MBEDTLS_BUILD_DIR%/../include -DMBEDTLS_LIBRARY=%MBEDTLS_BUILD_DIR%/library/%CONFIG%/mbedtls.lib -DMBEDX509_LIBRARY=%MBEDTLS_BUILD_DIR%/library/%CONFIG%/mbedx509.lib -DMBEDCRYPTO_LIBRARY=%MBEDTLS_BUILD_DIR%/library/%CONFIG%/mbedcrypto.lib -DENABLE_TESTING=%ENABLE_TESTING% -DENABLE_SAMPLES=%ENABLE_SAMPLES% .. -G %VS_VERSION%
cmake -DBUILD_SHARED_LIBS=%BUILD_SHARED_LIBS% -DCMAKE_WINDOWS_EXPORT_ALL_SYMBOLS=%BUILD_SHARED_LIBS% -DS2OPC_PUBSUB_ONLY=off -DS2OPC_CLIENTSERVER_ONLY=off -DWITH_PYS2OPC=%WITH_PYS2OPC% -DPYS2OPC_WHEEL_NAME=%PYS2OPC_WHEEL_NAME% -DS2OPC_NANO_PROFILE=%S2OPC_NANO_PROFILE% -DS2OPC_EVENT_MANAGEMENT=%S2OPC_EVENT_MANAGEMENT% -DS2OPC_NODE_MANAGEMENT=%S2OPC_NODE_MANAGEMENT% -DCMAKE_PREFIX_PATH=%CMAKE_LIBS_DIRS% -DMBEDTLS_INCLUDE_DIR="C:/Users/ext-ghanemyo/Desktop/mbedtls/include" -DMBEDTLS_LIBRARY="C:/Users/ext-ghanemyo/Desktop/mbedtls/build/mbedtls/lib/mbedtls.lib" -DMBEDX509_LIBRARY="C:/Users/ext-ghanemyo/Desktop/mbedtls/build/mbedtls/lib/mbedx509.lib" -DMBEDCRYPTO_LIBRARY="C:/Users/ext-ghanemyo/Desktop/mbedtls/build/mbedtls/lib/mbedcrypto.lib" -DENABLE_TESTING=%ENABLE_TESTING% -DENABLE_SAMPLES=%ENABLE_SAMPLES% .. -G %VS_VERSION%
@IF not %ERRORLEVEL% == 0 goto :EXIT_FAIL
