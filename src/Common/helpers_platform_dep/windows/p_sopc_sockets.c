 ...
 */
 ...
 #include <errno.h>
    ...
    #define TCP_USER_TIMEOUT 18  // Current value for windows, if supported
#endif
 ...
 SOPC_ReturnStatus SOPC_SocketAddress_GetNameInfo(const SOPC_Socket_Address* addr, char** host, char** service)
 {
     ...
     {
         int res = getnameinfo(addr->address.ai_addr, (socklen_t) addr->address.ai_addrlen, hostRes, NI_MAXHOST, serviceRes,
                                NI_MAXSERV, flags);
         if (0 != res)
         {
             status = SOPC_STATUS_NOK;
 ...
 static SOPC_ReturnStatus Socket_Configure(int sock, bool setNonBlocking)
 {
     ...
     if (sock != -1)
     {
         ...
         {
            u_long mode = 1;
            setOptStatus = ioctlsocket(sock, FIONBIO, &mode);
        }
                                         ...
                                         SOPC_Socket* sock)
 {
     ...
     status = Socket_Configure((int)socketImpl->sock, setNonBlocking);
     ...
     else
     {
        ...
        S2OPC_TEMP_FAILURE_RETRY(res, closesocket(socketImpl->sock));
        SOPC_Free(socketImpl);
        *sock = SOPC_INVALID_SOCKET;
     }
     return status;
 ...
 SOPC_ReturnStatus SOPC_Socket_Connect(SOPC_Socket sock, SOPC_Socket_AddressInfo* addr)
 {
     ...
     {
         S2OPC_TEMP_FAILURE_RETRY(connectStatus, connect(sock->sock, addr->addrInfo.ai_addr, (int)addr->addrInfo.ai_addrlen));
         if (connectStatus < 0)
         {
 ...
 SOPC_ReturnStatus SOPC_Socket_ConnectToLocal(SOPC_Socket from, SOPC_Socket to)
 {
     ...
     int addrlen = (int)addr.addrInfo.ai_addrlen;
 
    //  if (0 == getsockname(to->sock, addr.addrInfo.ai_addr, &addr.addrInfo.ai_addrlen))
    if (0 == getsockname(to->sock, addr.addrInfo.ai_addr, &addrlen))
     {
                                      ...
                                      uint32_t waitMs)
 {
     ...
     else
     {
        ...
        //  timeout.tv_usec = (suseconds_t)(1000 * (waitMs % 1000)); //posix
 ...
 SOPC_ReturnStatus SOPC_Socket_BytesToRead(SOPC_Socket sock, uint32_t* bytesToRead)
 {
     ...
     if (sock != SOPC_INVALID_SOCKET && bytesToRead != NULL)
     {
         ...
         S2OPC_TEMP_FAILURE_RETRY(res, ioctlsocket(sock->sock, FIONREAD, (u_long*) &nbBytes));
         if (res == 0 && nbBytes >= 0)
         {
