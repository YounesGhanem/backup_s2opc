...
static SOPC_ReturnStatus SOPC_UDP_Socket_AddrInfo_Get(int IPv6, const char* node, const char* port, SOPC_Socket_AddressInfo** addrs)
{
...
static SOPC_ReturnStatus SOPC_UDP_Socket_CreateNew(const SOPC_Socket_AddressInfo* addr, const char* interfaceName, int setReuseAddr, int setNonBlocking, SOCKET* sock)
{
...
int SetSendInterface(SOCKET s, struct addrinfo* iface)
{
    ...
    if (rc != SOCKET_ERROR)
    {
        ...
        PRINT_DEBUG("setsockopt(%d, IPPROTO_IP, IP_MULTICAST_IF, %d.%d.%d.%d:%d)\n", 
            (int)s,
            inaddr->sin_addr.S_un.S_un_b.s_b1,
...
// static SOPC_ReturnStatus SOPC_UDP_Socket_CreateNew(const SOPC_Socket_AddressInfo* addr, const char* interfaceName, int setReuseAddr, int setNonBlocking, SOPC_Socket* pSock)
// {
//     SOPC_ReturnStatus status = SOPC_STATUS_INVALID_PARAMETERS;
//     const int trueInt = 1;
//     int setOptStatus = -1;
//     if (addr != NULL && pSock != NULL)
//     {
//         SOPC_Socket result = (SOPC_Socket)malloc(sizeof(*result));
...
//         if (status == SOPC_STATUS_OK && setReuseAddr)
//         {
//             PRINT_DEBUG("setsockopt(%d, SOL_SOCKET, SO_REUSEADDR,..) \n", (int) result->sock);
...
//         SOPC_UDP_Socket_Set_MulticastTTL(result, DEFAULT_TTL);
...
//             int ret = getaddrinfo(interfaceName, NULL, &hints, &res);
//             if (ret != 0) {
...
//                     getnameinfo(tmp->ai_addr, (socklen_t)tmp->ai_addrlen, host, sizeof(host), NULL, 0, NI_NUMERICHOST);
//                     puts(host);
//                 }
...
localif.sin_addr.S_un.S_un_b.s_b4,
localif.sin_addr.S_un.S_un_b.s_b3,
localif.sin_addr.S_un.S_un_b.s_b2,
localif.sin_addr.S_un.S_un_b.s_b1,
(int)htons(localif.sin_port));

if (res == SOCKET_ERROR)
{
PRINT_DEBUG("Failed to Bind socket %d on port %d (%s)\n", (int) (*pSock)->sock, htons(localif.sin_port), gai_strerrorA(WSAGetLastError()));
...
SOPC_ReturnStatus SOPC_UDP_Socket_CreateToSend(SOPC_Socket_AddressInfo* destAddress, const char* interfaceName, bool setNonBlocking, SOPC_Socket* pSock)
{
...
SOPC_ReturnStatus SOPC_UDP_Socket_SendTo(SOPC_Socket pSock, const SOPC_Socket_AddressInfo* destAddr, SOPC_Buffer* buffer)
{
    //if (sock == INVALID_SOCKET || destAddr == NULL || buffer == NULL || buffer->position != 0)
    ...
    int res = sendto(pSock->sock, (const char*)buffer->data, buffer->length, 0, destAddr->addrInfo.ai_addr, (int)destAddr->addrInfo.ai_addrlen);
...
SOPC_ReturnStatus SOPC_UDP_Socket_ReceiveFrom(SOPC_Socket pSock, SOPC_Buffer* buffer)
{
    ...
    int recv_len = recvfrom(pSock->sock, (char*)buffer->data, buffer->current_size, 0, (struct sockaddr*)&si_client, &slen);
